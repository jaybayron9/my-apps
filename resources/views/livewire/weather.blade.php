<div x-show="open"  
    x-cloak
    class="fixed top-0 left-0 w-full h-full flex justify-center bg-gray-700/20">
    <div @click.away="open = false"
        class="min-w-[23%] max-w-md h-fit rounded-md p-4 mt-44 bg-gray-50">
        <button aria-label="Close" 
                @click="open = false"
                class="absolute top-3 right-3 p-3 bg-gray-300 hover:bg-gray-200 rounded-full">
            Exit
        </button>
        <h3>Check the weather forcast</h3> 
        <form wire:submit="forecast"
                class="flex gap-2">
            <input type="text" 
                    placeholder="City" 
                    wire:model="city" 
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            <Button type="submit" 
                    wire:loading.attr="disabled" 
                    class="flex gap-1 text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5">
                    <span wwire:loading.remove>Check</span>
                    <div role="status"
                        wire:loading 
                        wire:target="forecast">
                        <svg aria-hidden="true" class="w-5 h-5 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                            <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                        </svg>
                        <span class="sr-only">Loading...</span>
                    </div>
            </Button>
        </form>
        <div class="flex gap-2">
            <div>
                <input type="radio" 
                        checked 
                        name="temperature_type"
                        wire:model="temperature_type" 
                        id="celsius" 
                        value="celsius" />
                <label for="celsius">Celsius</label>
            </div>
            <div>
                <input type="radio"
                        name="temperature_type"
                        wire:model="temperature_type" 
                        id="farenheit" 
                        value="farenheit">
                <label for="farenheit">Farenheit</label>
            </div>
        </div>
        <div>
            <h4>{{ $forecast_data['location']['name'] . ', ' . $forecast_data['location']['region'] 
                ?? $forecast['location']['name'] . ', ' . $forecast['location']['region'] }}</h4>
            <p>Current temperature: {{ 
                    $temperature_type === 'celsius' 
                    ? $forecast['current']['temp_c'] . '°C'  
                    : $forecast['current']['temp_f'] . '°F' 
                }}</p>
            <p>Feels like: {{ 
                    $temperature_type === 'celsius'
                    ? $forecast['current']['feelslike_c'] . '°C'  
                    : $forecast['current']['feelslike_f'] . '°F'
                }}</p>
            <p>Max: {{ 
                    $temperature_type === 'celsius'
                    ? $forecast['forecast']['forecastday'][0]['day']['maxtemp_c'] . '°C' 
                    : $forecast['forecast']['forecastday'][0]['day']['maxtemp_f'] . '°F'
                }}, Min: {{ 
                        $temperature_type === 'celsius'
                        ? $forecast['forecast']['forecastday'][0]['day']['mintemp_c'] . '°C'  
                        : $forecast['forecast']['forecastday'][0]['day']['mintemp_c'] . '°F' 
                    }}</p>
            <p>{{ $forecast['current']['condition']['text'] }}</p>
            <img src="{{ $forecast['current']['condition']['icon'] }}" alt="Weather condition" class="h-20 w-20">
        </div>
    </div>
</div>